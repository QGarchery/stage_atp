...
\begin{lstlisting}[frame=single]
  ...
  Open Scope Z_scope.
\end{lstlisting}
Cela requiert l'installation de SMTCoq, disponible à l'adresse \textit{https://github.com/smtcoq/smtcoq}. On préférera la version utilisant native-coq \cite{native-coq}.
...
Il n'est pas possible d'écrire en Coq une fonction qui détermine la forme conjonctive d'un argument booléen. En effet, le type $bool$ n'ayant que deux constructeurs ($true$ et $false$), inspecter par \textit{pattern-matching} nous donnera un de ces deux constructeurs.\medbreak
...
Ce théorème de correction n'est applicable que si on a un terme de la forme $interp \,\, x$, forme que l'on peut obtenir en s'assurant que pour tout terme booléen $b$, l'interprétation de la réification de $b$ est convertible à $b$.\medbreak
...
\[ checker  \,\, cc \,\, t \, = \, true \quad \rightarrow \quad negb \,\,  (interp \, \, f) \]
...
L'emplacement courant, c'est-à-dire l'emplacement du tableau d'état à modifier lors d'un appel de $checker$, initialisé à 2 et est incrémenté à chaque appel de $checker$. C'est encore le même fonctionnement que les certificats de veriT. \medbreak
...
On peut aussi mentionner Sledgehammer \cite{sledgehammer_manual} qui est un outil développé pour l'assistant de preuve Isabelle afin d'y intégrer une interface sceptique avec des prouveurs automatiques. Sledgehammer a mis en place un sélectionneur de lemmes qui apprend quels lemmes il est judicieux d'envoyer aux prouveurs automatiques \cite{hol_selector}.\medbreak
