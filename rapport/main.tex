...
Le vérificateur définit en Coq un type \textit{crule} qui représente les \textit{trule} ainsi qu'une fonction $checker$. Nous allons définir ces termes puis voir comment la combinaison des deux permet de reproduire la sémantique des certificats de veriT (\ref{sortie}).
...
Pour enregistrer le résultat des règles précédentes on utilise un tableau de clauses appelé état. La fonction $checker$ implémente l'application des règles du certificat et modifie donc l'état à chaque nouvelle \textit{crule} rencontrée. Plus précisémment, $checker$ est une fonction Coq qui est définie récursivement sur son paramètre de type \textit{ccertif} et qui modifie l'état. À chaque appel de $checker$, une nouvelle \textit{crule} $cr$ est consommée, $checker$ met alors à jour l'état courant en fonction de $cr$.\\
...
\[ \forall f \,\, \forall cc. \quad checker  \,\, cc \,\, f \, = \, true \quad \Rightarrow \quad interp \, \, f \]
...
Chacun des constructeurs du type \textit{crule} contient aussi paramètre de position utilisé par la fonction $checker$ (\ref{checker}).
